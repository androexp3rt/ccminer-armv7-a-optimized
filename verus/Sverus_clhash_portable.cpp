...
FORCE_INLINE int32x4_t _mm_clmulepi64_si128_emu(const int32x4_t a, const int32x4_t &b, int imm)
{
 ...
 result = (int32x4_t)vmull_p64(vgetq_lane_u64(a, 1), vgetq_lane_u64(b,0)); 
...
int32x4_t precompReduction64_si128_port(int32x4_t A) {
	...
	int32x4_t Q3 = _mm_shuffle_epi8_emu(_mm_setr_epi8_emu(0, 27, 54, 45, 108, 119, 90, 65, (char)216, (char)195, (char)238, (char)245, (char)180, (char)175, (char)130, (char)153),
...
int32x4_t __verusclmulwithoutreduction64alignedrepeat_port2_1(int32x4_t *randomsource, const int32x4_t buf[4], uint64_t keyMask, uint16_t * __restrict fixrand, uint16_t * __restrict fixrandex)
{
...
//	keyMask >>= 4;
	...
	uint32_t prandex_idx = (selector >>32) & keyMask;
		...
		case 0:
		{
			...
			const int32x4_t temp2 = _mm_load_si128_emu(pbuf - (((selector & 1) << 1) - 1));
		...
		case 4:
		{
			...
			const int32x4_t temp22 = _mm_load_si128_emu(pbuf - (((selector & 1) << 1) - 1));
			const int32x4_t add12 = _mm_xor_si128_emu(temp12, temp22);
		...
		case 0xc:
		{
			...
			{
				const int32x4_t tempb3 = _mm_load_si128_emu(prandex);
				_mm_store_si128_emu(prandex, tempa2);
		...
		case 0x10:
		{
			...
			int32x4_t temp1 = _mm_load_si128_emu(pbuf - (((selector & 1) << 1) - 1));
			...
			const int32x4_t tempa3 = _mm_xor_si128_emu(tempa1, tempa2);

			const int32x4_t tempa4 = _mm_load_si128_emu(prandex);
			...
			#pragma clang loop unroll(full)
                        for ( int count = 1; count <= 8; count++ )
                        {
                                ...
                                //printf("selector %" PRIu64 "\n", selector);
                                                ...
                                                {
                                                        const int32x4_t temp2 = _mm_load_si128_emu(rounds & 1 ? pbuf : buftmp);  //rounds is odd
                                                ...
                                                {
                                                        int32x4_t temp2 = _mm_load_si128_emu(rounds & 1 ? buftmp : pbuf);  //rounds is odd
		...
		for ( int count = 1; count <= 8; count++ )
                {
                        if ( rounds >= 0 )
                        {
                                    if (selector & (((uint64_t)0x10000000) << rounds))
                                    {
                                        ...
                                        const int32x4_t temp2 = vld1q_s32((const int32_t *) (rounds & 1 ? pbuf : buftmp));
                                    ...
                                    else
                                    {
                                        ...
                                        const int32x4_t add1 = veorq_s32(onekey, temp2);
                ...
                const int32x4_t tempa3 = _mm_load_si128_emu(prandex);
                const int32x4_t tempa4 = _mm_xor_si128_emu(tempa3, acc);
...
//#pragma clang loop unroll(full)
		...
		default: 
		{
			...
			acc = _mm_xor_si128_emu(tempa3, acc);
			const int32x4_t temp4 = _mm_load_si128_emu(pbuf - (((selector & 1) << 1) - 1));
...
uint64_t verusclhash_port2_1(void * random, const unsigned char buf[64], uint64_t keyMask, uint16_t *  __restrict fixrand, uint16_t * __restrict fixrandex) {
    ...
    int32x4_t  acc = __verusclmulwithoutreduction64alignedrepeat_port2_1(rs64, string, keyMask, fixrand, fixrandex);
